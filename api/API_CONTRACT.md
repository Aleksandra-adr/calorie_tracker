...
## Справочник продуктов и автоподстановка (добавлено в `feature/product-search`)

Этот раздел **дополняет** контракт выше, ничего в нём не меняя: поля
`calories`/`proteins`/`fats`/`carbs` в `POST/PUT /meals` по-прежнему
задаются вызывающей стороной вручную (см. «Важное архитектурное решение»
выше) — ничего не стало вычисляться на сервере автоматически при создании
записи. Новые эндпоинты — это отдельный, необязательный помощник: они дают
клиенту (фронтенду) готовые числа, которые он может (но не обязан)
подставить в форму перед отправкой `POST /meals`.

Источник данных — небольшой встроенный справочник ~19 распространённых
продуктов «на 100 г» (`storage/products_seed.json`, загружается в память
модулем `storage/product_catalog.py`). Это не таблица в SQLite — справочник
статичен на время жизни процесса и не редактируется через API.

### 7. Поиск продукта по названию

```
GET /products?q=<строка>
```

- `q` — обязателен, минимум 1 символ. Регистронезависимый поиск по
  подстроке в названии продукта (подстрока может быть в любом месте
  названия, не только в начале).
- Нет совпадений — `200 OK`, пустой массив `[]` (НЕ 404) — так же, как и
  `GET /meals` за период без записей.
- Пустой/отсутствующий `q` — `422 Unprocessable Entity` (стандартная
  структура ошибок валидации pydantic).

Пример запроса: `GET /products?q=банан` или `GET /products?q=Банан` —
дают одинаковый результат.

Ответ `200 OK` — JSON-массив объектов **на 100 г продукта** (не порции!):
```json
[
...
Обратите внимание: поля здесь называются `protein`/`fat`/`carbs`
(единственное число) — так же, как в `calculations.portion.NutritionPer100g`,
а НЕ `proteins`/`fats` как в модели приёма пищи (`MealEntryBase`). Это
осознанное решение, чтобы форма ответа сама сигнализировала «это значения
на 100 г, а не порция» — путать их с `MealEntryResponse` не должно быть
удобно чисто по написанию полей.

### 8. Пересчёт калорий/БЖУ по весу для продукта из справочника

```
GET /products/portion?product_name=<строка>&weight_grams=<число>
```

- `product_name` — точное (без учёта регистра) название продукта, как оно
  вернулось из `GET /products` (`name` в ответе выше).
- `weight_grams` — вес порции в граммах, `>= 0` и `<= 100000`.
- Если продукт с таким именем не найден в справочнике — `404 Not Found`:
  `{"detail": "Product '...' not found in catalog"}`.
- Отсутствие/невалидность параметров — `422 Unprocessable Entity`.

Расчёт выполняется **единственной** функцией `calculations.calculate_portion`
(та же, что покрыта unit- и property-тестами в `tests/test_calculations.py`)
— этот эндпоинт существует именно для того, чтобы формула
`значение_на_100г * weight_g / 100` не дублировалась в JavaScript на
фронтенде.

Ответ `200 OK` — поля здесь уже называются как в `MealEntryBase`
(`calories`/`proteins`/`fats`/`carbs`, множественное число), потому что это
готовая порция, которую можно подставить прямо в форму/`POST /meals`:
```json
{
  "product_name": "Банан",
  "weight_grams": 150,
